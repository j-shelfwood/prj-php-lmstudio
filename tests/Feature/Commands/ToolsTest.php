...
test('it can execute a tool call', function () {
    ...
    expect($this->commandTester->getStatusCode())->toBe(0)
        ...
        ->and($display)->toContain('Fetching weather for: London')
        ->and($display)->toContain('Weather data:');
});

test('it handles missing tool handler', function () {
    ...
    expect($this->commandTester->getStatusCode())->toBe(1)
        ->and($display)->toContain('No handler registered for tool');
});
